...
SAML.prototype.validateExpiration = function (samlAssertion, version) {
  var self = this;
  const dates = self.getExpirationDates(samlAssertion)
  if (!dates) {
    return true
  }

  if (now < dates.notBefore || now > dates.notOnOrAfter)
    return false;
...
SAML.prototype.getAssertionID = function (samlAssertion) {
  const idAttributes = ['Id', 'ID', 'id', 'AssertionID'];
  for (const index in idAttributes) {
    const elemXpath = `//*[@*[local-name(.)='"${idAttributes[index]}"']='" + uri + "']`;
    const elem = xpath.select(elemXpath, samlAssertion);
    // Get the first occurence of an id attribute
    if (elem.length > 0) {
      return elem[0];
    }
  }
...
SAML.prototype.getExpirationDates = function (samlAssertion) {
  const self = this;
  const conditions = xpath.select(".//*[local-name(.)='Conditions']", samlAssertion);
  if (!conditions || conditions.length === 0) return null;

  const notBefore = new Date(conditions[0].getAttribute('NotBefore'));
  notBefore = notBefore.setMinutes(notBefore.getMinutes() - self.options.clockSkew);

  const notOnOrAfter = new Date(conditions[0].getAttribute('NotOnOrAfter'));
