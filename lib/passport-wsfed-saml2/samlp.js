...
function ignoreValidationFunction(data, done){
  return done();
}
...
var Samlp = module.exports = function Samlp (options, saml) {
  ...
  this.isValidAssertionID = this.options.isValidAssertionID || ignoreValidationFunction;
    ...
    self.isValidResponseID(responseXML.getAttribute('ID'), function(err){
      ...
      self.isValidInResponseTo(inResponseTo, function(err){
        ...
        self.extractAssertion(samlResponse, function (err, assertion) {
          ...
          const assertionID = self._saml.getAssertionID(assertion)
          const expirationDates = self._saml.getExpirationDates(assertion)

          self.isValidAssertionID({ assertionID, expirationDates }, function (err) {
            if (err) { return callback(err); }
